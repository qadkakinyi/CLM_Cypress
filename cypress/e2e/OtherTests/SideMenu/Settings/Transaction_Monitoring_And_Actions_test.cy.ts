/**
 * @testSuite Settings - Transaction Monitoring & Actions Navigation
 * @description Verifies navigation for each submenu item under Settings → Transaction Monitoring, ensuring correct page loads, expected heading appears, and loader is hidden.
 * @priority Medium
 * @owner QA Team
 * @tags regression, navigation, settings, transaction-monitoring, actions
 * @dependencies cypress, getByDataCy
 * @fileDescription Iterates through Transaction Monitoring & Actions routes and validates URL, heading text, and that loading spinner disappears.
 */
...
describe('Transaction Monitoring & Actions', ()=>{
    ...
    /**
     * @suite Transaction Monitoring & Actions
     * @description Loops through defined routes and validates navigation, URL, spinner, and page heading.
     * @prerequisites User logged in; sidebar menu expanded or pinnable; transaction-menu and transaction-links selectors present.
     */

    routes.forEach((route) => {
        /**
         * @scenario Visit Transaction Monitoring & Actions Page
         * @description Navigates to a specific Settings → Transaction Monitoring submenu item and validates its page.
         * @priority Medium
         * @testData index = route.index, expectedPath = route.route, expectedText = route.assertion
         * @steps Visit dashboard → Pin sidebar if unpinned.
         * @steps Open Settings → Transaction Monitoring menu.
         * @steps Click submenu item by index.
         * @steps Validate URL, wait for spinner to disappear, and confirm heading text.
         * @expectedResult Correct page loads with matching URL and heading; spinner is not visible.
         */
