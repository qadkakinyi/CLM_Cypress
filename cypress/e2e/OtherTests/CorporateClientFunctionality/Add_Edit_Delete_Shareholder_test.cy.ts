...
/**
 * @testSuite Shareholders/Partners
 * @description Validates CRUD operations and validation behavior for shareholder/partner entities
 * @priority High
 * @owner QA Team
 * @tags corporate, stakeholders, compliance
...
describe('Shareholders/Partners', () => {
     ...
     * @description Adds a shareholder with defined capacity and share allocation
     * @expectedResult Shareholder is successfully added
     */
    it('Adds Shareholder', () => {
        let clientName;
        ...
        cy.get('.left-secondary-menu .left-menu-items.main-menu li>sa-menu-item>a')
            .contains('span', 'Shareholders/Partners').click().wait(2000);
        ...
        cy.getByDataCy('weight-percentage').clear()
            .type(faker.number.int({ min: 1, max: 19 }).toString());

        cy.getByFormControlName('numberOfShares').wait(500).clear()
            .type(faker.number.int({ min: 1, max: 10 }).toString());
        ...
        cy.getBySel('dynamicSelectBoxDropdownGrid').find('[aria-rowindex="2"]').eq(1)
    ...
    /**
     * @scenario Exceed Authorized Capital Validation
     * @description Attempts to add shares beyond authorized capital to trigger validation
     * @expectedResult Error message: "The number of shares exceed the authorised capital."
     */
    it('Should error out if shares exceed Authorized capital', () => {
        cy.visit(location).wait(3000);

        cy.get('.left-secondary-menu .left-menu-items.main-menu li>sa-menu-item>a')
            .contains('span', 'Profile').click().wait(2000);
        cy.getByFormControlName('authorisedCapital').invoke('val').then(inputValue => {
            ...
            cy.getByDataCy('weight-percentage')
                .clear().type(faker.number.int({ min: 0, max: 19 }).toString());

            cy.getByFormControlName('numberOfShares')
                .clear().type(authorized_Capital + 1).toString();
    ...
    /**
     * @scenario Edit Shareholder
     * @description Updates shareholding percentage for existing shareholder
     * @expectedResult Shareholder is successfully updated
     */
    it('Edits Authorized Person', () => {
        cy.visit(location).wait(2000);
        cy.get('#gridClientShareholders .dx-icon-chevrondoubleright')
            .should('be.visible').eq(0).click({ force: true }).wait(2000);
        cy.get('#editClientShareholderForm');
        cy.getByDataCy('weight-percentage')
            .clear().type(faker.number.int({ min: 10, max: 49 }).toString());
     ...
     * @description Deletes the first shareholder listed in the client profile
     * @expectedResult Shareholder is deleted and confirmation is displayed
     */
    it('Deletes Authorized Person', () => {
        cy.visit(location).wait(3000);
        cy.get('#gridClientShareholders .dx-icon-trash')
            .should('be.visible').eq(0).click({ force: true }).wait(1500);
